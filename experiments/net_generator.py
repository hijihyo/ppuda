...
def main():
    ...
    for f in [h5_file, meta_file]:
        if os.path.exists(h5_file):
            raise ValueError(
                "File %s already exists. The script will exit now to avoid accidental overwriting of the file." % f
            )
    ...
    with h5py.File(h5_file, "w") as h5_data:
        ...
        while len(meta_data[split]["nets"]) < N:
            layers = int(
                np.random.randint(min_layers, max_layers + 1)
            )  # number of cells in total (convert to int to make it serializable)
            deep_layers = np.random.choice(deep_layers_all)  # a threshold to consider a deep network

            steps = int(
                np.random.randint(min_steps, (max_steps if layers <= deep_layers else medium_steps) + 1)
            )  # number of summation nodes in a cell
            genotype = sample_genotype(
                steps=steps,
                only_pool=bool(np.random.rand() > 0.5),  # True means no trainable layers in the reduction cell
                drop_concat=bool(np.random.rand() > 0.5)
                if steps > 1
                else False,  # drop some edges from the sum node to the final concat
            ...
            is_cse2 = (sum([n[0] == "cse" for n in genotype.normal]) > 1) or (
                sum([n[0] == "cse" for n in genotype.reduce]) > 1
            )  # training GHNs on networks with CSE often leads to NaN losses, so we will avoid them

            is_conv = (
                sum([n[0].find("conv") >= 0 for n in genotype.normal + genotype.reduce]) > 0
            )  # at least one simple conv op

            is_conv_large = (sum([n[0] in ["conv_5x5", "conv_7x7"] for n in genotype.normal]) > 1) or (
                sum([n[0] in ["conv_5x5", "conv_7x7"] for n in genotype.reduce]) > 1
            )  # dense convolutions are memory consuming, so we will avoid them
            ...
            if split == "bnfree":
                ...
                norm = (
                    np.random.choice(["bnorm", None])
                    if layers <= (min_layers + 1) and steps <= medium_steps
                    else "bnorm"
                )
            ...
            net_args = {
                "stem_type": stem_type,
                "stem_pool": bool(
                    stem_type == 0 and np.random.rand() > 0.5
            ...
            for dset_name in ["cifar10", "imagenet"]:
                model = Network(
                    C=32,  # default number of channels
                ...
                if dset_name == "cifar10":
                    try:
                        ...
                        print(
                            "\n%d: unable to construct the graph: it is likely to be disconnected"
                            % len(meta_data[split]["nets"]),
                            "has_none={}, genotype={}".format(has_none, net_args["genotype"]),
                            flush=True,
                        )
                        print(e, "\n")
            ...
            if (idx + 1) % 100 == 0 or idx >= N - 1:
                ...
                print(
                    "N={} nets created: \t {}-{} nodes (mean\u00B1std: {:.1f}\u00B1{:.1f}) "
