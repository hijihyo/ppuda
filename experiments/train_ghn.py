...
from ppuda.utils import Trainer, capacity
...
def main():
    ...
    train_queue, val_queue, num_classes = image_loader(
        args.dataset,
        args.data_dir,
        test=False,
    ...
    if state_dict is not None:
        try:
            ...
            print(
                "WARNING: optimizer is not available in the checkpoint, available keys: ",
                state_dict.keys(),
                "error:",
                e,
                "\n",
    ...
    for epoch in range(start_epoch, args.epochs):
            ...
            while not upd:
                try:
                    ...
                    for nets_args in graphs.net_args:
                        net = Network(is_imagenet_input=is_imagenet, num_classes=num_classes, light=True, **nets_args)
                    ...
                    if oom or is_nan:
                        if failed_batches > len(train_queue) // 50:
                            print(
                                "Out of patience (after %d attempts to continue), "
        ...
        if args.save:
            ...
            torch.save(
                {
                    "state_dict": (ghn.module if args.multigpu else ghn).state_dict(),
