...
Example:
    ...
    python experiments/sgd/train_net.py --split search --arch 35133 -d cifar10 --epochs 600 --cutout \
        --drop_path_prob 0.2 --auxiliary

    # Fine-tune DARTS initialized with the parameters predicted by GHN-2-ImageNet on 100-shot CIFAR-10:
    python experiments/sgd/train_net.py --arch DARTS --epochs 50 -d cifar10 --n_shots 100 --wd 1e-3 --init_channels 48 \
        --layers 14 --ckpt ./checkpoints/ghn2_imagenet.pt
...
from ppuda.utils import Trainer, adjust_net, capacity, infer, init, pretrained_model
...
def main():
    ...
    train_queue, valid_queue, num_classes = image_loader(
    ...
    try:
        ...
        try:
            ...
            if args.arch is not None:
                arch = int(args.arch)
            deepnets = DeepNets1M(split=args.split, nets_dir=args.data_dir, large_images=is_imagenet, arch=arch)
    ...
    if isinstance(arch, str):
        model = adjust_net(
            eval(
                "torchvision.models.%s(pretrained=%d,num_classes=%d)"
    ...
    model = init(model, orth=args.init.lower() == "orth", beta=args.beta, layer=args.layer, verbose=args.debug > 1)
