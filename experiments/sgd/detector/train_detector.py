...
Example:
    ...
    python experiments/sgd/detector/train_detector.py --arch DARTS --ckpt ./checkpoints/ghn2_imagenet.pt \
        --init_channels 48 --layers 14
...
import ppuda.deepnets1m.genotypes as genotypes  # noqa: F401
...
from ppuda.utils import capacity, pretrained_model
...
def main():
    ...
    dataset = PennFudanDataset(
        os.path.join(args.data_dir, args.dataset), transforms=T.Compose([T.ToTensor(), T.RandomHorizontalFlip(0.5)])
    )
    ...
    optimizer = torch.optim.SGD(
        [p for p in model.parameters() if p.requires_grad], lr=args.lr, momentum=args.momentum, weight_decay=args.wd
    )
...
def get_backbone(args):
    try:
        ...
        net_args = {
            "C": args.init_channels,  # 48 if genotype == DARTS else 128
            "genotype": genotype,
            ...
            "stem_type": 1,
        }  # assume that the ImageNet-style stem is used by default
    except:  # noqa: E722
        deepnets = DeepNets1M(split=args.split, nets_dir=args.data_dir, large_images=True, arch=args.arch)
        assert len(deepnets) == 1, "one architecture must be chosen to train"
        graph = deepnets[0]
        net_args, _ = graph.net_args, graph.net_idx
    ...
    if isinstance(net_args["genotype"], str):
        ...
        model = Network(num_classes=num_classes, is_imagenet_input=True, is_vit=False, **net_args)
...
def get_model_detection(args, num_classes):
    ...
    if args.arch not in ["", None]:
        ...
        roi_pooler = torchvision.ops.MultiScaleRoIAlign(
            featmap_names=["0"], output_size=7, sampling_ratio=2  # 14 if im_size is not None else 7,
        )

        # put the pieces together inside a FasterRCNN model
        model = FasterRCNN(
            backbone, num_classes=num_classes, rpn_anchor_generator=anchor_generator, box_roi_pool=roi_pooler
        )

        if hasattr(model.backbone, "genotype") and model.backbone.genotype == ViT:
            model.transform = GeneralizedRCNNTransform(
                int(800 / 2.5), int(1333 / 2.5), [0.485, 0.456, 0.406], [0.229, 0.224, 0.225]
            )
