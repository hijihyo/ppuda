...
def main():
    ...
    assert (
        n_train == len(properties["val_acc"]["test"]) == 500
    ...
    if os.path.exists(cache_file):
        ...
        def extract_graph_embeddings(graphs_queue):
            ...
            for graphs in tqdm(graphs_queue):
                assert len(graphs) == 1, (
                    "only one architecture per batch is supported in the evaluation mode",
                    len(graphs),
                )
                net_args, _ = graphs.net_args[0], graphs.net_inds[0]

                model = Network(
                    num_classes=1000 if is_imagenet else 10, is_imagenet_input=is_imagenet, **net_args
                ).eval()
                x.append(
                    ghn(model, graphs.to_device(ghn_device), return_embeddings=True)[1]
                    .mean(0, keepdim=True)
                    .data.cpu()
                    .numpy()
                )
        ...
        x_train = extract_graph_embeddings(
            DeepNets1M.loader(split="val", virtual_edges=virtual_edges, large_images=is_imagenet)
        )
        x_test = extract_graph_embeddings(
            DeepNets1M.loader(split="test", virtual_edges=virtual_edges, large_images=is_imagenet)
        )
        assert len(x_train) == len(x_test) == n_train, (x_train.shape, x_test.shape, n_train)
        x_search = extract_graph_embeddings(
            DeepNets1M.loader(split="search", virtual_edges=virtual_edges, large_images=is_imagenet)
        )
    ...
    for prop, splits in properties.items():
        ...
        model = SVR(**clf.best_params_).fit(
            x, np.concatenate((y_train, y_test))
        )  # using the best params found with seed=None
        ...
        print(
            "Architecture with the best {} (prediction={:.3f}) in the SEARCH split is {} ({} test samples)".format(
                prop.upper(), y_pred[best_arch], best_arch, len(y_pred)
            )
        )

        # the best (in the sense of a given property) architecture can be trained by running (see vision/train_net.py
        # for more examples):
