...
from ppuda.utils import AvgrageMeter, adjust_net, capacity, infer
...
assert (
    ghn.num_classes == num_classes
), "the evaluation image dataset and the image dataset that the GHN was trained on do not match"

is_imagenet = args.dataset == "imagenet"

graphs_queue = DeepNets1M.loader(
    split=args.split, nets_dir=args.data_dir, large_images=is_imagenet, virtual_edges=virtual_edges, arch=args.arch
)
...
for graphs in graphs_queue:
    ...
    if isinstance(net_args["genotype"], str):
        model = adjust_net(
            eval("torchvision.models.%s(num_classes=%d)" % (net_args["genotype"], num_classes)), is_imagenet
        ).eval()
    else:
        model = Network(num_classes=num_classes, is_imagenet_input=is_imagenet, **net_args).eval()
...
print(
    "\ndone for {} nets: (avg\u00B1standard error) top1={:.1f}\u00B1{:.1f}, top5={:.1f}\u00B1{:.1f}".format(
        top1_all.cnt, top1_all.avg, top1_all.dispersion, top5_all.avg, top5_all.dispersion
    )
)
