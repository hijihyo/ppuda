...
from ppuda.utils import default_device, set_seed
...
def init_config(mode="eval", parser=None, verbose=True, **kwargs):
    ...
    try:
        ...
        if verbose:
            print(
                "WARNING: PyTorch version {} is used, but version >= 1.9 is "
    ...
    # Data args
    parser.add_argument(
        "-d", "--dataset", type=str, default="cifar10", help="image dataset: cifar10/imagenet/PennFudanPed."
    )
    ...
    parser.add_argument(
        "-D", "--data_dir", type=str, default="./data", help="where image dataset and DeepNets-1M are stored"
    )
    parser.add_argument(
        "--test_batch_size", type=int, default=1 if is_detection else 64, help="image batch size for testing"
    )
    ...
    parser.add_argument(
        ...
        "--num_workers",
        type=int,
        default=8 if is_imagenet else (4 if is_detection else 0),
    ...
    parser.add_argument(
        "--debug",
        type=int,
        default=kwargs.pop("debug", 1),
        help="the level of details printed out, 0 is the minimal level.",
    )
    parser.add_argument("-C", "--ckpt", type=str, default=None, help="path to load the network/GHN parameters from")
    ...
    parser.add_argument(
        "--split",
        type=str,
        default=kwargs.pop("split", "train" if is_train_ghn else "predefined"),
    ...
    parser.add_argument(
        ...
        "--imsize",
        type=int,
        default=kwargs.pop("imsize", 224 if is_imagenet else 32),
    ...
    if is_train:
        ...
        parser.add_argument(
            ...
            "--log_interval",
            type=int,
            default=10 if is_detection else 100,
            help="number of training iterations when print intermediate results",
        )
        parser.add_argument(
            "-S", "--save_dir", type=str, default="./checkpoints", help="where to put all trained data and stuff"
        )
        ...
        parser.add_argument(
            "--lr_steps",
            type=str,
            default=kwargs.pop("lr_steps", "200,250" if is_train_ghn else "30,60"),
        ...
        parser.add_argument(
            "--layers",
            type=int,
            default=kwargs.pop("layers", 1 if is_train_ghn else (14 if is_imagenet else 20)),
        ...
        if is_train_ghn:
            ...
            parser.add_argument("-M", "--num_nets", type=int, default=10**6, help="number of training architectures")
            parser.add_argument(
                "-v",
                "--virtual_edges",
                type=int,
                default=kwargs.pop("virtual_edges", 1),
            ...
            parser.add_argument(
                "-n", "--weight_norm", action="store_true", default=False, help="normalize predicted weights"
            )
            parser.add_argument(
                "-m",
                "--meta_batch_size",
                type=int,
                default=kwargs.pop("meta_batch_size", 1),
            ...
            parser.add_argument(
                "--max_shape",
                type=str,
                default=kwargs.pop("max_shape", None),
            ...
            parser.add_argument(
                "--init_channels",
                type=int,
                default=48 if is_imagenet else 36,
        ...
        if args.max_shape is None:
            ...
            if len(c) == 1:
                ...
                raise NotImplementedError(
                    "max_shape must be a string of 1, 2 or 4 integers separated by commas. "
        ...
        if args.multigpu:
            ...
            if n_devices > 1:
                ...
                if is_train_ghn:
                    ...
                    assert (
                        args.meta_batch_size % n_devices == 0
                ...
                if verbose:
                    print(
                        "multigpu argument is ignored: > 1 cuda devices necessary, "
