...
ViT = Genotype(
    normal=[("none", 0), ("msa", 1)], normal_concat=[2], reduce=[("none", 0), ("avg_pool_3x3", 1)], reduce_concat=[2]
)
...
def sample_genotype(steps=1, only_pool=False, allow_none=True, drop_concat=True, allow_transformer=False):
    ...
    if only_pool:
        ...
        alphas_reduce[
            ...
            -1000
        )  # prevent sampling operators with learnable params to sample the architectures similar to the best DARTS cell
    ...
    def _parse(weights):
        ...
        for i in range(steps):
            ...
            edges = sorted(
                range(i + 2),
                key=lambda x: -max(
                    W[x][k] for k in range(len(W[x])) if (k != PRIMITIVES_DARTS_EXT.index("none") or allow_none)
                ),
            )[:2]
            for j in edges:
                k_best = None
    ...
    if drop_concat:
        ...
        for i in range(2 + steps - multiplier, steps + 2):
            if (
                i == steps + 1 or torch.rand(1).item() > 0.5
            ):  # always add the last otherwise the features from the previous sum nodes will be lost
                concat.append(i)
    else:
