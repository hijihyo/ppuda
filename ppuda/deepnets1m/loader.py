...
from ..deepnets1m import genotypes  # noqa: F401
from ..utils import adjust_net, rand_choice
from .genotypes import PRIMITIVES_DEEPNETS1M, from_dict
from .graph import Graph, GraphBatch
from .net import Network
from .ops import *  # noqa: F401, F403

# to fit larger meta batches into GPU memory (decreasing this number further may create a bias towards smaller
# architectures)
...
class DeepNets1M(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.split = split
        assert self.split in ["train", "val", "test", "search", "wide", "deep", "dense", "bnfree", "predefined"], (
            "invalid split",
            self.split,
        )
        ...
        if self.split == "predefined":
            ...
            self.h5_file = os.path.join(
                nets_dir, "deepnets1m_%s.hdf5" % (split if split in ["train", "search"] else "eval")
            )
        ...
        if self.verbose:
            print(
                "loaded {}/{} nets with {}-{} nodes (mean\u00B1std: {:.1f}\u00B1{:.1f})".format(
                    len(self.nets),
                    n_all,
                    self.nodes.min().item(),
                    self.nodes.max().item(),
    ...
    def __getitem__(self, idx):
        if self.split == "predefined":
            ...
            graph = self._init_graph(
                self.h5_data[self.split][str(idx)]["adj"][()], self.h5_data[self.split][str(idx)]["nodes"][()], n_cells
            )
    ...
    def _get_predefined(self):
        ...
        for idx, arch in enumerate(["resnet50", "ViT"]):  # Here, other torchvision models can be added
            num_classes = (
                1000 if self.large_images else 10
            )  # the exact number should not be important at the graph construction stage
            if arch == "resnet50":
                model = adjust_net(
                    eval("torchvision.models.%s(num_classes=%d)" % (arch, num_classes)), large_input=self.large_images
                )
                args = {"genotype": arch}
            else:
                args = {
...
class NetBatchSampler(torch.utils.data.BatchSampler):
    ...
    def check_batch(self, batch):
        return self.max_nodes_batch is None or self.sampler.data_source.nodes[batch].sum() <= self.max_nodes_batch
