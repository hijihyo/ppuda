...
class Graph:
        ...
        if model is not None:
            if isinstance(model, torchvision.models.vision_transformer.VisionTransformer):
                raise NotImplementedError(
                    "Official PyTorch VisionTransformer module is not supported in the graph construction process. "
                    "Use the deepnets1m.net.Network class and deepnets1m.genotypes.ViT to construct it."
                )

            sz = (
                model.expected_input_sz if hasattr(model, "expected_input_sz") else 224
            )  # assume ImageNet image width/heigh by default
                    ...
                    if u[0] is not None:
                        if hasattr(u[0], "variable"):
                            ...
                            if type(module) not in MODULES:
                                print(
                                    "WARNING: unrecognized layer {}, params = {}, type = {}".format(
                                        name,
                                        sum(
                                            [
                                                p.numel()
                            ...
                            leaf_nodes.append(
                                {"id": u[0], "param_name": name, "attrs": {"size": var.size()}, "module": module}
                            )
    ...
    def _add_virtual_edges(self, ve_cutoff=50):
        ...
        for node in range(self.n_nodes):
            assert node in length or self._nodes[node]["param_name"].startswith("pos_enc"), (
                "not all nodes have a path to the input",
                node,
                self._nodes[node],
            )
    ...
    def _construct_features(self):
        ...
        for node_ind, node in enumerate(self._nodes):
            ...
            if node["module"] is not None:
                ...
                if self.n_cells > 1:
                    # Add cell id to the names of pooling layers, so that they will be matched with proper modules in
                    # Network
            ...
            if isinstance(attrs, dict):
                if "size" in attrs:
                    ...
                    if "kernel_size" in attrs:
                        ...
                        # Pytorch 1.9+ is required to correctly extract pooling attributes, otherwise the default
                        # pooling size of 3 is used
                        sz = (1, 1, 3, 3)
    ...
    def visualize(
        ...
        def color(i):
            return cm.jet(int(np.round(255 * i / n_primitives)))

        primitive_colors = {
            PRIMITIVES_DEEPNETS1M[ind_org]: color(ind_new) for ind_new, ind_org in enumerate(primitives_order)
        }
        ...
        node_groups = {
            ...
            "max_pool": {
                "style": {"edgecolors": "k", "linewidths": 1, "node_shape": "o", "node_size": 1.75 * node_size}
            },
            "glob_avg": {
                "style": {"edgecolors": "gray", "linewidths": 0.5, "node_shape": "o", "node_size": 2 * node_size}
            },
        ...
        if vis_legend:
            node_feat = torch.cat((torch.tensor([n_primitives]).view(-1, 1), torch.tensor(primitives_order)[:, None]))
