    ...
    def __init__(
        ...
        if self._is_vit:
            ...
            self.stem0 = nn.Sequential(
                conv_layer(light)(
                    3, C // 2, kernel_size=ks, stride=2 if is_imagenet_input else 1, padding=ks // 2, bias=False
                ),
        ...
        if is_w or is_b:
            ...
            if is_w:
                key = module_name + (
                    ".layer_scale"
                    if is_layer_scale
                    else (".in_proj_weight" if is_proj_w else (".pos_embedding.weight" if is_pos_enc else ".weight"))
                )
                w = (
                    m.layer_scale
                    if is_layer_scale
                    else (m.in_proj_weight if is_proj_w else (m.pos_embedding if is_pos_enc else m.weight))
                )
                layered_modules[cell_ind][key] = {
                    "param_name": key,
                    "module": m,
                    "is_w": True,
