...
class Trainer:
    ...
    def update(self, models, images, targets, ghn=None, graphs=None):
        ...
        with torch.cuda.amp.autocast(enabled=self.amp):
            ...
            if isinstance(self.device, (list, tuple)):
                # Multigpu training
                assert isinstance(models, (list, tuple)) and isinstance(
                    models[0], (list, tuple)
                ), "models must be a list of lists"
                ...
                models_per_device = len(
                    models[0]
                )  # assume that on the first device the number of models is >= than on other devices
                for ind in range(models_per_device):  # for index withing each device
                    ...
                    outputs = parallel_apply(
                        model_replicas, image_replicas[: len(model_replicas)], None, self.device[: len(model_replicas)]
                    )  # forward pass at each device in parallel
        ...
        if torch.isnan(loss):
            raise RuntimeError(
                f"the loss is {loss}, unable to proceed. This issue can often be fixed by restarting the script and "
                "loading the saved checkpoint using the --ckpt argument."
            )
    ...
    def log(self, step=None):
        ...
        if step_ % self.log_interval == 0 or step_ >= self.n_batches - 1:
            ...
            print(
                "batch={:04d}/{:04d} \t {} \t {:.2f} sec/batch ({:.1f} min left) ".format(
                    step_, self.n_batches, metrics, speed, speed * (self.n_batches - step_) / 60
                ),
                flush=True,
            )
