...
class ConvDecoder(nn.Module):
    def __init__(self, in_features=64, hid=(128, 256), out_shape=None, num_classes=None):
        ...
        self.fc = nn.Sequential(nn.Linear(in_features, out_features), get_activation("relu"))
        self.register_buffer(
            "cols_1d", torch.arange(0, out_features).view(hid[0], out_shape[2], out_shape[3]), persistent=False
        )
        self.register_buffer("cols_4d", torch.arange(0, ch_prod), persistent=False)

        conv = []
        for j, n_hid in enumerate(hid):
            n_out = ch_prod if j == len(hid) - 1 else hid[j + 1]
            conv.extend([nn.Conv2d(n_hid, n_out, 1), get_activation(None if j == len(hid) - 1 else "relu")])

        self.conv = nn.Sequential(*conv)
        self.class_layer_predictor = nn.Sequential(get_activation("relu"), nn.Conv2d(out_shape[0], num_classes, 1))
    ...
    def forward(self, x, max_shape=(1, 1, 1, 1), class_pred=False):
        ...
        if sum(max_shape[2:]) < sum(self.out_shape[2:]) and len(self.fc) == 2:
            ...
            x = self.fc[1](
                F.linear(x, self.fc[0].weight[ind], self.fc[0].bias[ind]).view(N, -1, max_shape[2], max_shape[3])
            )  # N,128,a,b
        else:
...
class MLPDecoder(nn.Module):
    def __init__(self, in_features=32, hid=(64,), out_shape=None, num_classes=None):
        super(MLPDecoder, self).__init__()

        assert len(hid) > 0, hid
        self.out_shape = out_shape
        self.num_classes = num_classes
        self.mlp = MLP(in_features=in_features, hid=(*hid, np.prod(out_shape)), activation="relu", last_activation=None)
        self.class_layer_predictor = nn.Sequential(get_activation("relu"), nn.Linear(hid[0], num_classes * out_shape[0]))
