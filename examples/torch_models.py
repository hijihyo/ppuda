...
kw_args = (
    {"aux_logits": False, "init_weights": False} if arch == "googlenet" else {}
)  # ignore auxiliary outputs in googlenet for this example

# Predict all parameters (any network from torchvision.models can be used)
model = ghn(
    adjust_net(eval("torchvision.models.%s(num_classes=%d, **kw_args)" % (arch, num_classes)), large_input=is_imagenet)
)
